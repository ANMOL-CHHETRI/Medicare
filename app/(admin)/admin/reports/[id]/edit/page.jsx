...
export default function EditReportPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const r = rJson.report
        setForm({ patient: r.patient?.id || "", doctor: r.doctor?.id || "", title: r.title || "", description: r.description || "", findings: r.findings || "" })
      } catch (e) {
  ...
  async function onSubmit(e) {
    e.preventDefault()
    setError("")
    const res = await fetch(`/api/reports/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
    if (res.ok) {
      router.push("/admin/reports")
    } else {
      const body = await res.json().catch(() => ({}))
      setError(body.message || "Failed to update report")
    }
  }
